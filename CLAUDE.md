...
- **Dark is the default theme**, as of 2026-09-05. `ThemeService` (`core/theme/theme.service.ts`) toggles a `.dark` class on `<html>` that every token in `styles.css` reacts to, persists the choice to `localStorage['rally.theme']`, and — on a native Capacitor build — sets the OS status bar style to match, since the status bar otherwise keeps whatever style the OS defaulted to and can land unreadable. A small inline script in `index.html` applies the class before Angular boots, to avoid a light-then-dark flash on load; it defaults to dark the same way `ThemeService.readInitialTheme()` does when nothing is stored. `rally-theme-toggle` (`shared/components/theme-toggle/`) is the user-facing switch.
...
- **Google sign-in (native, iOS/Android):** the web flow above cannot work in a Capacitor app — there is no browser location for a page redirect to return to, and separately, Google's own policy blocks completing sign-in inside an embedded WebView at all (it detects the user agent and refuses). Fixed 2026-09-10, diagnosed from the symptom "signing in with Google bounces out to the production GitHub Pages URL instead of staying in the app": `loginWithGoogle()` special-cases `Capacitor.isNativePlatform()` — it asks Supabase for the auth URL without navigating (`signInWithOAuth({ ..., skipBrowserRedirect: true })`) and opens it in the system browser via `@capacitor/browser`'s `Browser.open()`, with `redirectTo` set to the `rally://auth/callback` custom scheme (registered in `ios/App/App/Info.plist`'s `CFBundleURLTypes` and as an intent-filter on `MainActivity` in `android/app/src/main/AndroidManifest.xml`; the latter's existing `launchMode="singleTask"` is what makes the deep link land on the running activity instead of a fresh one). The return trip is caught by `AppComponent`'s `appUrlOpen` listener (`@capacitor/app`), which calls `AuthService.completeNativeOAuthRedirect(url)`. This project's client runs the *implicit* flow, confirmed live via Logcat: the deep link carries `#access_token=...&refresh_token=...` in the URL fragment, not a `?code=...` in the query string — there is no PKCE code to exchange, despite that being the first (wrong) assumption this fix shipped with. `completeNativeOAuthRedirect()` parses the fragment by hand and calls `supabase.auth.setSession()` directly with those tokens, since Supabase's own `detectSessionInUrl` auto-parse (what makes the web flow work with no extra code) only ever watches `window.location`, which a native deep link never touches. It then makes the same hasProfile → `/` or `/register` decision `AuthCallbackPageComponent` makes on web, duplicated rather than routed through that component since there's no `ActivatedRoute` to feed it from a native event. `AppComponent` also dedupes on the exact redirect URL it last handled — `@capacitor/app` is known to fire `appUrlOpen` twice for one deep link on some Android versions, which would otherwise show a spurious error after an already-successful sign-in. Requires `rally://auth/callback` to also be added to Supabase's Redirect URLs allow-list (see docs/flows/login.md's Prerequisites) — without it this fails the same silent-fallback-to-Site-URL way the web flow does when its own entry is missing.

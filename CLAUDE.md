...
- Notifications: the bell in the topbar (`features/notifications/notifications-bell/`) is backed by a real, generic `notifications` table delivered live via Supabase Realtime — see `NotificationsRepository` (`features/notifications/data/notifications.repository.ts`) and its `NotificationsService` facade. `AppNotification.kind` plus a `data` rendering-payload snapshot keep it reusable; a `NotificationKind → { icon, textKey, detailKey }` map in the bell component is the single place to wire up a new kind's copy (translation keys live under `notifications.kinds.<camelCaseKind>`). Only `trip_host_volunteered` exists today, emitted by `TripsRepository.volunteer()` toward the traveller. Clicking a notification currently always opens a DM thread with its `actorId` — add kind-specific routing in `NotificationsBellComponent.select()` if a future kind needs something else. Do not add a `message` kind here: the messages widget already has its own live unread badge.

### Mock today

- Courts, open matches, match history/results, passport countries/courts/achievements and community stats still originate in `RallyDataService` / `rally-dataset.ts`.

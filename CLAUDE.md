...
RALLY is a Portuguese-first social tennis app, and as of 2026-09-11 it is a **web app only** — one Angular codebase, one target, deployed to GitHub Pages. It was briefly wrapped as native iOS/Android via Capacitor; that was removed wholesale on 2026-09-11 (see "Web-Only, As Of 2026-09-11" below) because the founder chose to focus on the web app first and decide on a native route later. Mobile still matters a lot — most of the UI is designed phone-first and is used in a mobile browser — it just means a mobile *browser*, not a packaged app. Read `PRODUCT.md` as well for product intent, user rules, and UX decisions — including the brand capitalisation/gender rule for the app name in user-facing and business copy. This file is the technical source of truth.
...
## Web-Only, As Of 2026-09-11

**Everything Capacitor/iOS/Android was removed from this repo on 2026-09-11**, at the founder's
request: the plan is to get the web app right first and decide separately, later, how it becomes a
native app. Nothing about the native route is "temporarily disabled" — it is gone, and if it comes
back it starts from `npx cap init` on whatever the codebase looks like then.

What was deleted: the `ios/` and `android/` project folders, `capacitor.config.ts`, the root
`assets/` folder (icon/splash source for `@capacitor/assets`, unused by the Angular build, which
serves from `public/`), all ten `@capacitor/*` packages, and the `cap:sync`/`cap:ios`/`cap:android`/
`android`/`ios` npm scripts. Five source files lost their native branch:

- `app.ts` — the `appUrlOpen` deep-link listener and its `handleNativeOAuthRedirect()`, plus the
  `AuthService`/`Router`/`ToastService`/`TranslationService` injections that existed only for it.
  It is now just the Twemoji start-up call.
- `auth.service.ts` — `NATIVE_OAUTH_REDIRECT_URL` (`rally://auth/callback`), the
  `skipBrowserRedirect` + `Browser.open()` branch in `loginWithGoogle()`, and
  `completeNativeOAuthRedirect()`. Google sign-in is the plain web redirect flow again, handled
  end to end by `AuthCallbackPageComponent`.
- `geolocation.service.ts` — `@capacitor/geolocation` → `navigator.geolocation` directly, wrapped
  in a promise. Same public API (`supported`, `locating`, `locate()`, `isGoodFix`, `isUsableFix`),
  same accuracy thresholds, and the error mapping is now only the browser's numeric codes (1
  denied / 3 timeout) since the `OS-PLUG-GLOC-*` string codes were Capacitor's. A side benefit:
  the console stub in "Testing Location-Gated Flows" below now patches the exact function the
  service calls.
- `share.service.ts` — `PRODUCTION_WEB_ORIGIN` and `@capacitor/share` are gone; `urlFor()` always
  resolves against `document.baseURI` and `share()` is `navigator.share` with the clipboard
  fallback. Both native bugs those existed to fix (localhost links, a silent share button) were
  WebView problems that cannot occur in a real browser.
- `theme.service.ts` — no more `StatusBar.setStyle()`; the effect just toggles `.dark` on `<html>`.

**`env(safe-area-inset-*)` CSS was deliberately kept.** It is a plain web standard, not a Capacitor
feature: it resolves to `0` in an ordinary browser tab (so it costs nothing on desktop or in mobile
Safari/Chrome) and to a real value when the app is installed to a phone's home screen as a
standalone PWA, which is the most likely next step for "mobile" here. Removing it would have been a
layout change with no upside. The elements that use it — the topbar and its separate `fixed top-0`
backdrop strip, the bottom nav, dialogs, toasts, the FAB, the media lightbox, the auth pages, the
feed's fixed offsets — all pair it with a hardcoded rem amount where they need one, and are correct
as they stand. Two rules carried over from the 2026-09-06 debugging pass still apply: never wrap it
in a `max(Xrem, env(...))` floor as a "safety net" (that forces empty space where the inset is
genuinely 0, which is a worse bug than whatever it guards against), and measure with
`getComputedStyle(el).paddingTop` before assuming the inset is the problem.

Also still true, and unrelated to the platform question: the topbar and the feed's scope-tabs bar
hide on scroll by plain translate only. Fading them with `opacity` was tried and pulled by the user
on 2026-09-06 — fiddly to tune and it never felt right. Don't re-attempt without an explicit ask.
...
src/app/
  core/
    ...
    services/      Toasts, dialogs, Twemoji, avatar generation, sharing, UI state
    theme/         ThemeService: toggles the `.dark` class on <html>; dark is the default
  features/
    ...
    feed/          Real posts, the public /posts/:id page, the composer's image crop
    courts/matches/passport/  Real (see Real vs Mock Data)
    world/         Real trip intents; the destinations/activity showcase is still mock
...
- `ui-dialog` is the composer shell: bottom sheet on mobile, centred card from `sm` up, with the
  ...
  `max-h-[90vh]` cap for a short body. `[centered]="true"` opts out of the bottom-sheet-on-mobile
  default entirely, for a short action-list body (e.g. a post's "..." menu) that should read as a
  floating card on every screen size rather than a composer — see the Feed entry above. The four
  centred-modal shells duplicated elsewhere (world host list, player invite, profile avatar,
  profile password) are a separate, still-uncollapsed pattern — see Planned Cleanup.
...
- **Dark is the default theme**, as of 2026-09-05. `ThemeService` (`core/theme/theme.service.ts`) toggles a `.dark` class on `<html>` that every token in `styles.css` reacts to, persists the choice to `localStorage['rally.theme']`, and nothing else — it used to also set the native status bar style, which went with Capacitor on 2026-09-11. A small inline script in `index.html` applies the class before Angular boots, to avoid a light-then-dark flash on load; it defaults to dark the same way `ThemeService.readInitialTheme()` does when nothing is stored. `rally-theme-toggle` (`shared/components/theme-toggle/`) is the user-facing switch.
...
29. `0031_landing_waitlist.sql` adds `landing_waitlist` (`email`, optional `name`/`country`/`city`, `locale`, `created_at`), `anon`-insertable only. Not used by this app at all — it's the public `rally-landing` project's launch-notification form, in this Supabase project only because it's the same project, sharing infra rather than intent. Deliberately mute: no select/update/delete policy for anyone but the project owner via the Supabase dashboard.
30. `0032_landing_waitlist_email_unique.sql` swaps `landing_waitlist_email_idx` from a `lower(email)` expression index to a plain index on `email`. The expression index broke the landing page's `on_conflict=email` upsert on its very first live submission (400 Bad Request, Postgres 42P10) — PostgREST's `on_conflict` inference needs a constraint on the literal column named, and `lower(email)` doesn't qualify. Case-insensitive dedup is preserved anyway since `Waitlist.join()` (in `rally-landing`) lowercases the address before sending, and this table has no other writer.
31. `0033_landing_waitlist_resubmit.sql` re-asserts `landing_waitlist`'s INSERT policy idempotently (`drop policy if exists` + recreate — the next live test after `0032` came back 401/42501 "new row violates row-level security policy", meaning the policy from `0031` either never fully applied or was since dropped; rather than diagnose which, this just guarantees the end state) and adds an UPDATE policy alongside it, same permissive `using (true) with check (true)` shape, intended to make `INSERT ... ON CONFLICT DO UPDATE` work for resubmitting an email. **Superseded by `0034` below** — confirmed live (both policies verified present via `pg_policies`) that this still wasn't enough.
...
- The Feed: user-authored posts (text and/or one photo/video, uploaded to the `feed-media` Storage bucket) scoped by "In my city" / "In my country" / "In the world", and a single like toggle. There is currently no reply/comment affordance on posts (removed after initial ship — DM-from-a-post may return later). See `PostsRepository` (`features/feed/data/posts.repository.ts`). The old synthetic `kind`-tagged activity (match/court/milestone auto-posts) was retired with it — deliberately deferred until courts/matches/notifications are themselves real, at which point they can post into `posts` the same way. Trip-announcement posts (`post.trip` populated, `trip_intent_id` not null) are the first case of this pattern actually landing: they carry no text/media of their own, render from the linked trip intent instead, are visible to everyone in "In the world" plus to viewers whose own city/country matches the trip's *destination* (not the traveller's home — the useful audience is potential hosts), and show a "Ser anfitrião" volunteer action (reusing `TripsRepository.volunteer()`) only to viewers in the destination country who aren't the traveller and haven't already volunteered. As of 2026-09-04, match/trip/venue posts — `FeedCardComponent.isAutomaticPost()` — no longer show the like button for anyone (they're system-generated announcements, not authored content); they can still be deleted the same way as before (a trip/match/venue post only via deleting the linked record, which cascades).
  As of 2026-09-07, sharing and a public post-detail page are real, closing the gap the note above used to describe. `/posts/:id` (`PostDetailPageComponent`, `features/feed/pages/post-detail-page/`) is registered in `app.routes.ts` outside the app shell and outside `authGuard` — deliberately public, since posts are already readable by the `anon` role (0011_posts.sql) and the point of a share link is reaching someone without an account yet. `ShareService` (`core/services/share.service.ts`) resolves an absolute in-app URL against `document.baseURI`, not `location.origin` (the GitHub Pages deploy isn't served from the domain root), and backs three exits from `rally-feed-card`'s "..." menu: the share sheet (`navigator.share`, which mobile browsers hand to the OS and desktop ones mostly don't implement at all), falling back to clipboard on *any* rejection — including a spurious `AbortError` some browser/OS combinations throw when there's simply no registered share target, not only on a genuine user cancel — an explicit "copy link", and sending the link as a plain-text DM to a recent conversation. That last path is why `MessagesService`/the messages widget now recognise a message whose text is *exactly* a post link (`ShareService.postIdFromShareLink()`) and render it as a distinct tappable bubble instead of a raw URL — a caption typed alongside a pasted link is deliberately left as an ordinary message. Posts can also be reported now (`0036_post_reports.sql`, mirroring `0027_court_reports.sql` exactly: mute table until a backoffice exists, one open report per reporter per post via a partial unique index, visible only to the reporter) via the same menu; `PostsRepository.report()` reports a duplicate (`23505`) back as `'already'`, not a failure. The composer's image attach goes through a crop step first (`rally-feed-image-crop`, `features/feed/components/feed-image-crop/`, backed by `ngx-image-cropper`) that fixes a 4:5 frame before upload — video skips straight to `FeedService.attachMedia()` since cropping it would mean re-encoding, not just a canvas draw; see `FeedService.selectMedia()`/`confirmCrop()`/`cancelCrop()`. `ui-dialog` gained a `[centered]` input for this work: the "..." menu and its report/share sub-views read as a floating card on every screen size, not the mobile bottom-sheet default (see Architecture Rules). The card itself was also redesigned this pass: no more per-post `rounded-2xl border bg-card` box — it's a borderless, edge-to-edge stream now (`-mx-5`/full column width on media, a bottom-border divider per post instead), Instagram-style, and trip/match/venue announcement posts got the same full-bleed `aspect-4/5` panel footprint as a media post (an icon-in-a-circle plus headline replaces what used to be a compact inline text block). This is a deliberate departure from "rounded cards" in Styling and UX, scoped to the feed specifically — don't read it as a regression to fix.
  **Still a real, unsolved gap, not a bug:** a shared post link has no per-post preview (no photo, no caption) when it lands in WhatsApp/iMessage/etc. — this is a static SPA on GitHub Pages with no server-side rendering, so every route serves the same static `index.html`, which as of 2026-09-11 at least carries generic Open Graph/Twitter Card tags (`hero-rally.jpg`, a fixed title/description) so a shared link shows *something* branded instead of nothing — but a link to one specific post can't show that post's own content without either SSR or an edge function serving crawlers a distinct response per post id, neither of which exists. Revisit only as a deliberate infrastructure decision, not a quick patch. (Two other sharing bugs lived here until 2026-09-11 — links resolving to `capacitor://localhost/...`, and a share button that silently did nothing — both Capacitor WebView problems, both gone with Capacitor itself. They cannot occur in a real browser, so don't re-add the workarounds.)
...
- When a header needs to hide on scroll, use the existing numeric `hideOffset`, not a boolean threshold with CSS transform transitions. On iOS, momentum scroll keeps firing scroll events after the finger lifts, which can race an unrelated state reset (e.g. opening the nav drawer) that assumes the header is still visible — `AppShellComponent.onMainScroll()` short-circuits and freezes `lastScrollTop` while the drawer is open, for exactly this reason.
- **A third visitor state, beyond signed-in and observer.** `AuthService.isObserver()` is only true for the app's own read-only guest mode — it is `false` for someone who isn't signed in at all, which includes any visitor to the public `/posts/:id` page (outside `authGuard`, see the Feed entry above). Gating an action on `!isObserver()` therefore reads as "yes, allowed" for that visitor even though they have no uid and no session, and a write then fails against RLS with no clear error. `FeedCardComponent` had this exact bug once `/posts/:id` existed and fixed it by gating every action (like, host, join, report) on `!!this.auth.currentUserId()` instead — truthy-uid is the correct check for "may act", `!isObserver()` is not, anywhere a component might render outside the authenticated shell.
- **The inverse of the trap above: a truthy uid that observer mode should override.** Choosing "Entrar como olheiro" while a real-but-incomplete session was still alive (a Google sign-in abandoned before finishing registration — see the Google sign-in entries above) used to leave that session's uid sitting in `currentUserId()` right alongside `isObserver()=true`. `authGuard`'s own `uid && !hasProfile(uid)` check — written before this hybrid state was possible — took the truthy uid at face value and force-redirected a brand-new observer into finishing someone else's half-done sign-up the moment they opened the shell. Fixed 2026-09-10: `AuthService.loginAsObserver()` now signs out and clears `_session` before setting `isObserver`, so the ambiguous state can no longer exist; `authGuard` also checks `!isObserver()` first as a second line of defence, since a bare truthy-uid check is exactly the pattern this whole section warns about, and observer is precisely the case a bare check can't see.
...
- **A `fixed`-position dialog nested inside an animated ancestor can get trapped inside it.** An element with `animation-fill-mode: both` (as `animate-slide-in` has) keeps its transform "in effect" — an identity matrix, not the literal `none` — for as long as the element exists, which makes it a containing block for any `position: fixed` descendant. `rally-feed-card`'s "..." menu (`ui-dialog`) is rendered as a sibling of the animated `<article>`, not nested inside it, for exactly this reason — nested, it was confined to the card's own box instead of covering the viewport. Any dialog placed inside a list item with an entrance animation needs the same check.
...
- **The centred-modal shell is still copied four times** (world host list, player invite, profile
  avatar, profile password). `ui-dialog` gained a `[centered]="true"` input on 2026-09-07 (see
  Architecture Rules), but that was built for the feed's "..." menu specifically and keeps the icon
  close button and backdrop-click-close — it doesn't fold in these four, which still differ enough
  (`px-4`, `max-w-md`/`max-w-sm`, a text `✕`, no backdrop-click close) that doing so would mean
  either a third variant or changing how they look. Worth doing, but it is a design decision, not a
  mechanical one.
...
**Lint** — `npm run lint` currently reports **0 errors and 7 warnings**, all `max-lines` (see the
Cleanup Pass section for what the other ~705 were and where they went). The files over the
300-line limit (`skipBlankLines`/`skipComments`, so this is smaller than a raw `wc -l`):
`courts.repository.ts` (434), `matches.repository.ts` (389), `matches.service.ts` (386),
`profile-page.component.ts` (385), `feed.service.ts` (382, up from 351 with the 2026-09-07
crop/report/share work), `posts.repository.ts` (367, up from 335, same reason), and
`register-page.component.ts` (309) — that last one is not new from the feed work, it just hadn't
been re-measured since crossing the line (last touched in the 2026-09-06 auth fix). Each needs a
judgement call about what to split off rather than a mechanical fix — the obvious row-type/mapper
layers have already been extracted.

**Bundle** — the initial bundle is ~1.81 MB against a 1 MB budget (up from ~1.77 MB before
`ngx-image-cropper` and the feed work), so every build warns. The obvious suspects are the country
dataset and the new cropper dependency; worth measuring before assuming. Either trim it or raise
the budget deliberately rather than leaving a warning everyone learns to ignore.

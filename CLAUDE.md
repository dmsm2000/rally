...
2. Build rich score/stats entry (per-set scores, `MatchStat` rows) — `matches.sets jsonb` already supports it schema-wise, only the completion UI is a minimal winner-picker today (singles only; doubles has no completion UI yet at all).

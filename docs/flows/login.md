...
3. **Supabase Dashboard** → Authentication → URL Configuration → make sure "Redirect URLs" includes
   `http://localhost:4200/auth/callback` (dev) and the equivalent in production. Without the entry,
   the `redirectTo` sent by `AuthService.loginWithGoogle()` is rejected and Supabase falls back to
   whatever "Site URL" is configured, silently landing the user there instead of back on
   `/auth/callback` — a failure mode that looks like "sign-in worked but went to the wrong page",
   not like an error.

Until this is done, the "Continuar com Google" button always shows Flow 14 (error).

**No native/Capacitor variant of this flow exists any more.** An iOS/Android build used to need a
`rally://auth/callback` custom scheme, `@capacitor/browser` and a deep-link listener, because
Google refuses to complete sign-in inside an embedded WebView. All of that was removed on
2026-09-11 when the project went web-only (see CLAUDE.md, "Web-Only, As Of 2026-09-11"), along with
the `rally://auth/callback` entry's reason to exist — it can be dropped from the Supabase Redirect
URLs allow-list too, though leaving it there is harmless.
